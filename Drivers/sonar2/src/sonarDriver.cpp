...
{	
	...
	cfsetspeed(&options, BAUDRATE);						// Set the baud rates to 115200...###############################.h!!!
	tcsetattr(fd, TCSANOW, &options);     		// Set the new options for the port...
	fcntl(fd, F_SETFL, FNDELAY);					// non-blocking low level read
	
	
tcsetattr(fd, TCSANOW, &options);// old thingy
...
/*Commented out wed.28.2011
	...
	*/ 
    ...
    timeout.tv_sec  = 1;
    timeout.tv_usec = 5000; // 30000 for gps worked
    ...
    else
    {
		...
		if (FD_ISSET(fd, &input)){
			...
			//puts("READ");
			 rBptr = &returnBuffer[0]; // old thingy
		}
...
void makePacket(int command)
{
	...
	if(command == mtReBoot || command == mtSendVersion || command == mtSendBBUser || command == mtStopAlive)
	{
		...
		unsigned char sendBuffer[14] = {	0x40, 		//Header
						0x30, 
						0x30, 
						0x30, 
						0x38, 
	...
	{		
		...
		unsigned char sendBuffer[18] = {	0x40, 		//Header
						0x30, 
						0x30, 
						0x30, 
						0x43, 
					...
					unsigned int ADlow, unsigned int gain, unsigned int ADInterval, unsigned int numBins)
{
	...
	if ( ADInterval < MIN_AD_INTERVAL )
	{
		...
		//ADInterval = MIN_AD_INTERVAL;
					...
					//Step 1, setup
	unsigned char sendBuffer[82] = { 	0x40,						//Header
